import turtle
def triangle(x, y, a, color):
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.fillcolor(color)
    turtle.pencolor("white")
    turtle.pensize(2)
    turtle.begin_fill()
    turtle.left(90)
    turtle.fd(a)
    turtle.right(135)
    turtle.fd((2*a**2)**0.5)
    turtle.left(225)
    turtle.fd(a)
    turtle.end_fill()
    turtle.up()
    turtle.home()

def star(x, y, a, color):
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.color(color)
    turtle.pensize(2)
    turtle.begin_fill()
    for i in range(0,5):
        turtle.fd(a)
        turtle.right(144)
    turtle.end_fill()
    turtle.up()

def circle(x, y, r, color):
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.color(color)
    turtle.begin_fill()
    turtle.circle(r)
    turtle.end_fill()
    turtle.up()

def rectangle(x, y, a, b, color):
    turtle.color(color)
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.begin_fill()
    turtle.forward(a)
    turtle.right(90)
    turtle.forward(b)
    turtle.right(90)
    turtle.forward(a)
    turtle.right(90)
    turtle.forward(b)
    turtle.right(90)
    turtle.end_fill()

def square(x, y, a, color):
    turtle.fillcolor(color)
    turtle.pencolor("white")
    turtle.pensize(2)
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.begin_fill()
    turtle.forward(a)
    turtle.right(90)
    turtle.forward(a)
    turtle.right(90)
    turtle.forward(a)
    turtle.right(90)
    turtle.forward(a)
    turtle.right(90)
    turtle.end_fill()
    turtle.up()
    turtle.home()

def paral(x, y, a, b, r1, r2, color):
    turtle.fillcolor(color)
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.pencolor('white')
    turtle.pensize(2)
    turtle.begin_fill()
    for i in range(2):
        turtle.forward(a)
        turtle.left(r1)
        turtle.forward(b)
        turtle.left(r2)
    turtle.end_fill()
    turtle.up()

def trap(x, y, a, b, c, r1, r2, color):
    turtle.fillcolor(color)
    turtle.up()
    turtle.setposition(x, y)
    turtle.down()
    turtle.pencolor('white')
    turtle.pensize(2)
    turtle.begin_fill()
    turtle.forward(a)
    turtle.left(r1)
    turtle.forward(b)
    turtle.left(r2)
    turtle.forward(c)
    turtle.left(r2)
    turtle.forward(b)
    turtle.left(r1)
    turtle.end_fill()

def tank(x, y):
    turtle.speed(100)
    circle(x + 0, y + 80, 38, "black")
    rectangle(x - 95, y + 130, 150, 80, "green")
    turtle.left(90)
    triangle(x -125, y + 0, 50, "green")
    rectangle(x + 77, y + 70, 30, 40, "black")
    rectangle(x + 78, y + 69, 28, 38, "green")
    rectangle(x - 126, y + 51, 202, 53, "black")
    rectangle(x -125, y + 50, 200, 50, "green")
    circle(x -100,y + 0, 23, "black")
    circle(x -50, y + 0, 23, "black")
    circle(x + 0, y + 0, 23, "black")
    circle(x + 50, y + 0, 23, "black")
    circle(x -100, y + 18, 5, "grey")
    circle(x -50, y + 18, 5, "grey")
    circle(x + 0, y + 18, 5, "grey")
    circle(x + 50, y + 18, 5, "grey")
    rectangle(x -190, y + 110, 100, 20, "green")
    rectangle(x -190, y + 110, 10, 20, "gray")
    rectangle(x -105, y + 80, 10, 5, "gray")
    circle(x + 0, y + 80, 35, "green")
    star(x -20, y + 100, 40, "red")
    turtle.left(225)
    triangle(x -210, y + 35, 50, "gray")
    turtle.up()
    turtle.home()

def picture_1(x1, y1):
    turtle.speed(100)
    turtle.left(45)
    triangle(x1 + 0, y1,200, "red")
    turtle.left(135)
    triangle(x1,y1 + 0,200, "yellow")
    turtle.left(225)
    triangle(x1 + 0, y1,100, "pink")
    turtle.left(90)
    triangle(x1 + 100*2**0.5, y1 - 100*2**0.5,144, "blue")
    paral(x1 -100*2**0.5, y1 - 100*2**0.5,200*(2**0.5)-145, 100, 45, 135,"green")
    turtle.left(45)
    square(x1 + 0, y1+ 0,98,"orange")
    turtle.right(45)
    triangle(x1 + 49*2**0.5, y1 + 49*2**0.5, 102, "purple")


def bird(x, y):
    turtle.speed(100)
    turtle.right(135)
    triangle(x + 0, y+ 0, 65, 'red')
    turtle.left(45)
    triangle(x+ 0, y + 0, 85, 'green')
    turtle.right(25)
    triangle(x + 57, y + 57, 55, 'blue')
    circle(x - 78, y + 37, 20, 'brown')
    turtle.left(145)
    triangle(x-78, y + 37, 25, 'purple')
    circle(x - 82, y + 60, 3, 'white')
    turtle.left(270)
    triangle(x - 10, y + 89, 50, 'orange')

def person(x, y):
    turtle.speed(100)
    turtle.right(225)
    square(x+0, y+0, 50, 'green')
    turtle.left(180)
    triangle(x-0.5, y-0.5, 85, 'blue')
    turtle.right(90)
    triangle(x+0.5, y-0.5, 85, 'red')
    turtle.right(90)
    turtle.right(60)
    paral(x-0.5, y-85.5, 65, 55, 50, 130, 'orange')
    turtle.left(75)
    paral(x+0.5, y-85.5, 55, 65, 50, 130, 'brown')
    turtle.left(145)
    triangle(x-80, y-218, 50, 'red')
    turtle.left(15)
    triangle(x+86, y-214, 50, 'red')

def vertolet(x, y):
    turtle.right(45)
    triangle(x + 0,y + 0,100*(2**1/2),'blue')
    turtle.right(225)
    triangle(x + 143,y + 0,100*(2**1/2),'blue')
    turtle.right(135)
    triangle(x + 110,y + 107,55,'blue')
    turtle.right(135)
    triangle(x + 30,y + 107,55,'blue')
    turtle.right(135)
    triangle(x + 2,y - 3,58,'blue')
    turtle.right(315)
    triangle(x - 38,y - 40,58,'blue')
    turtle.right(45)
    square(x - 100,y + 45,50,'blue')
    turtle.right(0)
    star(x - 50,y - 12,25,'red')

def lodka(x, y):
    turtle.right(45)
    trap(x + 0, y + 0, 50,50,50,30,30,'brown')
    turtle.right(210)
    triangle(x+120,y+50,70,'red')
    turtle.right(225)
    triangle(x + 156, y + 87, 120, 'red')
    turtle.right(45)
    rectangle(x + 159, y + 85, 59,48, 'green')
    turtle.right(315)
    triangle(x + 70, y + 170, 50, 'blue')
    turtle.right(270)
    triangle(x + 70, y + 20, 100, 'green')
    turtle.up()
    turtle.home()

def parovoz(x, y):
    rectangle(x + 0, y + 0, 150,70,'red')
    circle(x + 150, y - 70, 35, 'red')
    rectangle(x + 115, y + 30, 40 , 30, 'gray')
    rectangle(x + 105, y + 50, 60,20,'black')
    rectangle(x + 0, y - 70, 150, 20, 'gray')
    rectangle(x - 60, y + 60, 60, 150, 'blue')
    circle(x - 30, y - 130, 35, 'black')
    circle(x + 20, y - 130, 20, 'black')
    circle(x + 60, y - 130, 20, 'black')
    circle(x + 100, y - 130, 20, 'black')
    paral(x + 145, y - 120, 30, 30, 60,60,'gray')
    circle(x + 120, y - 100, 20, 'black')
    turtle.home()

def fish(x, y):
    turtle.width(4)
    turtle.right(45)
    square(x + 0,y + 0, 50, 'red')
    turtle.right(180)
    triangle(x + 37, y + 65, 100,'blue')
    turtle.left(90)
    triangle(x + 37, y - 136, 100, 'green')
    paral(x -84,y -36,50,75,130,50,'yellow')
    turtle.right(90)
    triangle(x - 84,y -36,50,'purple')
    turtle.right(270)
    triangle(x - 84,y -84,50,'orange')
    turtle.right(225)
    triangle(x + 87,y -35,70,'pink')

def rocket(x1, y1):
    turtle.right(135)
    triangle(x1+0,y1+100,50,'green')
    turtle.left(135)
    triangle(x1+33.5,y1-6,97.5,'orange')
    turtle.left(180)
    triangle(x1+35,y1+64,70,'red')
    turtle.right(45)
    triangle(x1-35,y1-76,97.5,'blue')
    turtle.right(45)
    square(x1-36.5,y1-77,50,'pink')
    turtle.right(227)
    triangle(x1-37.5, y1-150, 50,'purple')
    turtle.right(90)
    paral(x1+35,y1-72,75,60,50,130,'green')


turtle.screensize(1920, 1080)
tank(50, 200)
picture_1(0, 0)
bird(300, -300)
person(-300,50)
vertolet(0,-300)
lodka(200,225)
parovoz(-350, -250)
fish(-300,300)
rocket(300, 50)
turtle.done()
